...
class ApiBaseService implements ApiBaseServiceInterface
{
    ...
    public function singleSignOnResponse(
        $response,
        $http_status = HttpStatusCode::SUCCESS
    ) {
        return response()->json($response, $http_status);
    ...
    public function sendSuccessResponseChtBackend(
        $result,
        $message,
        $pagination = [],
        $http_status = HttpStatusCode::SUCCESS,
     ...
     * @return JsonResponse
     */
    public function sendErrorResponseChtBackend($message, $errorMessages = [], $status_code = HttpStatusCode::VALIDATION_ERROR)
    {
        $response = [
            'success' => false,
            'code' => $status_code,
